			...
			if (groupType === "MULTI") {
				for (let l = 0; l < languages.length; l++) {
	    			...
	    			for (let j = 0; j < formFields.length; j++) {
	    				...
	    				fieldName = formFields[j].name+"-"+languages[l].code;
	    				switch(fieldType) {
		    	    		case "TXT": {
		    	    			let resultTXT = validateFormFieldTXT(formFields[j],inputFields[fieldName],"",langRequired);
		    		        	if (resultTXT.isValid == false) {
		    		        		isValidTmp = resultTXT.isValid;
		    		        	}
		    		        	errorMapTmp = Object.assign({}, errorMapTmp, resultTXT.errorMap);
		    	    		...
		    	    		case "SLT": {
		    	    			let resultSLT = validateFormFieldSLT(formFields[j],inputFields[fieldName],"",langRequired);
		    		        	if (resultSLT.isValid == false) {
		    		        		isValidTmp = resultSLT.isValid;
		    		        	}
		    		        	errorMapTmp = Object.assign({}, errorMapTmp, resultSLT.errorMap);
				...
				for (let j = 0; j < formFields.length; j++) {
    				...
    				switch(fieldType) {
	    	    		case "TXT": {
	    	    			let resultTXT = validateFormFieldTXT(formFields[j],inputFields[formFields[j].name]);
	    		        	if (resultTXT.isValid == false) {
	    		        		isValidTmp = resultTXT.isValid;
	    		        	}
	    		        	errorMapTmp = Object.assign({}, errorMapTmp, resultTXT.errorMap);
	    	    		...
	    	    		case "SLT": {
	    	    			let resultSLT = validateFormFieldSLT(formFields[j],inputFields[formFields[j].name]);
	    		        	if (resultSLT.isValid == false) {
	    		        		isValidTmp = resultSLT.isValid;
	    		        	}
	    		        	errorMapTmp = Object.assign({}, errorMapTmp, resultSLT.errorMap);
