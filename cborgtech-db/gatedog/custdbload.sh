...
load_base(){
	...
	echo "Loading toasthub client_db.sql to ${1}"
   	mysql -h ${2} -u ${3} --password=${4} ${1} < ../toasthub/client_db.sql
   	echo "Loading gatedog client_db.sql to ${1}"
   	mysql -h ${2} -u ${3} --password=${4} ${1} < client_db.sql
	...
	echo "Loading customer_db_data.sql to ${1}"
	mysql -h ${2} -u ${3} --password=${4} ${1} < customer_db_data.sql
	echo "Done Loading db ${1}"
	}
	
all() {
	load_base gatedog_client1 localhost cyborg c7b8rg
	}
