...
CREATE TABLE `menu_item_values`
	...
	`route_web` varchar(512),
	`route_native` varchar(512),
	`icon_web` varchar(512),
	`icon_native` varchar(512),
	`lang` varchar(5) NOT NULL,
	`rendered` bit(1) NOT NULL DEFAULT 1,
	`is_active` bit(1) DEFAULT 1,
	`is_archive` bit(1) DEFAULT 0,
...
INSERT INTO `note_catagory` (`name`) VALUES ('Default');
...
CREATE TABLE `service_class`
	...
	`perm_right` varchar(2) NOT NULL DEFAULT 'D',
	`is_active` bit(1) DEFAULT 1,
	`is_archive` bit(1) DEFAULT 0,
	`is_locked` bit(1) DEFAULT 0,
	`lockowner_id` bigint(20) DEFAULT NULL,
